...
public class BackendClientService : IBackendClientService
{
    ...
    public async Task<bool> SendHeartbeatAsync(string deviceId, string? ipAddress = null)
    {
        try
        {
            ...
            _logger.LogDebug("💓 Heartbeat gönderiliyor: DeviceId={DeviceId}, BackendUrl={BackendUrl}, IpAddress={IpAddress}", 
                deviceId, _config.BackendBaseUrl, ipAddress ?? "null");
            
            var response = await _httpClient.PostAsJsonAsync("/api/agents/heartbeat", request);
            
            if (response.IsSuccessStatusCode)
            {
                _logger.LogDebug("✅ Heartbeat başarılı: DeviceId={DeviceId}, StatusCode={StatusCode}", 
                    deviceId, response.StatusCode);
                return true;
            }
            
            var errorContent = await response.Content.ReadAsStringAsync();
            _logger.LogWarning("⚠️ Heartbeat başarısız: DeviceId={DeviceId}, StatusCode={StatusCode}, Error={Error}", 
                deviceId, response.StatusCode, errorContent);
            return false;
        }
        catch (HttpRequestException ex)
        {
            _logger.LogError(ex, "❌ Heartbeat HTTP hatası: DeviceId={DeviceId}, BackendUrl={BackendUrl}, Message={Message}", 
                deviceId, _config.BackendBaseUrl, ex.Message);
            return false;
        }
        catch (TaskCanceledException ex) when (ex.InnerException is TimeoutException)
        {
            _logger.LogWarning("⏱️ Heartbeat timeout: DeviceId={DeviceId}, BackendUrl={BackendUrl}", 
                deviceId, _config.BackendBaseUrl);
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ Heartbeat gönderme hatası: DeviceId={DeviceId}, BackendUrl={BackendUrl}", 
                deviceId, _config.BackendBaseUrl);
            return false;
        }
    }

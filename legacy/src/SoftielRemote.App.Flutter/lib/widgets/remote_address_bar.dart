...
import '../providers/connection_service_provider.dart';
...
class _RemoteAddressBarState extends ConsumerState<RemoteAddressBar> {
  ...
  void didChangeDependencies() {
    ...
    // Connection status değişikliklerini dinle (sadece bir kez)
    if (_statusSubscription == null) {
      final connectionService = ref.read(connectionServiceProvider);
      _statusSubscription = connectionService.onStatusChange.listen((status) {
        if (mounted) {
          ref.read(appStateProvider.notifier).setConnectionStatus(status);
          setState(() {
            _isConnecting = status == ConnectionStatus.pending || 
                           status == ConnectionStatus.connecting;
          });
          
          if (status == ConnectionStatus.connected) {
            // Bağlantı başarılı - video ekranına geç
            // TODO: Video ekranına geç
      ...
      _errorSubscription = connectionService.onError.listen((error) {
        if (mounted) {
  ...
  Future<void> _handleConnect() async {
    ...
    if (deviceId.isEmpty) {
      ref.read(notificationProvider.notifier).showWarning(
        'Lütfen bir bağlantı kodu giriniz',
        duration: const Duration(seconds: 2),
      );
      return;
    }
    
    // Kendi device ID'sine bağlanmayı engelle
    final deviceInfo = ref.read(appStateProvider).deviceInfo;
    if (deviceId == deviceInfo.deviceId) {
      ref.read(notificationProvider.notifier).showError(
        'Kendi bilgisayarınıza bağlanamazsınız',
    ...
    // Connection status listener ekle
    final connectionService = ref.read(connectionServiceProvider);
    StreamSubscription? statusSubscription;
    
    statusSubscription = connectionService.onStatusChange.listen((status) {
      if (mounted) {
        setState(() {
          // Sadece pending veya connecting durumunda loading göster
          _isConnecting = status == ConnectionStatus.pending || 
                         status == ConnectionStatus.connecting;
        });
        
        // Bağlantı tamamlandığında veya hata olduğunda subscription'ı kapat
        if (status == ConnectionStatus.connected || 
    ...
    errorSubscription = connectionService.onError.listen((error) {
      if (mounted) {
        setState(() {
          _isConnecting = false;
        });
    ...
    try {
      ...
      final success = await connectionService.connect(
        deviceId,
        requesterName: deviceInfo.deviceId, // Şimdilik device ID kullan
      );
  ...
  Widget build(BuildContext context) {
    ...
    final isPendingOrConnecting = connectionInfo.status == ConnectionStatus.pending ||
                  ...
                  child: Focus(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: isPendingOrConnecting 
                            ? connectionInfo.status.displayName
                            : 'Bağlantı Kodu Giriniz',
                        ...
                        suffixIcon: Row(
                          ...
                          children: [
                            ...
                            Material(
                              color: Colors.transparent,
                              child: InkWell(
                                onTap: (isConnected || _isConnecting) ? null : _handleConnect,
                                borderRadius: BorderRadius.circular(6),
                                child: Container(
                                  width: 36,
                                  height: 36,

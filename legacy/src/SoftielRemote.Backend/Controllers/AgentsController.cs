...
public class AgentsController : ControllerBase
{
    ...
    [HttpPost("register")]
    // [EnableRateLimiting("AgentRegisterPolicy")] // Geçici olarak devre dışı
        ...
        [FromBody] AgentRegistrationRequest request)
    {
        ...
        {
            _logger.LogError(ex, "❌ Agent kayıt sırasında exception oluştu: {ExceptionType}, {Message}, {StackTrace}", 
                ex.GetType().Name, ex.Message, ex.StackTrace);
            
            // Inner exception varsa onu da logla
            if (ex.InnerException != null)
            {
                _logger.LogError(ex.InnerException, "❌ Inner exception: {ExceptionType}, {Message}", 
                    ex.InnerException.GetType().Name, ex.InnerException.Message);
            }
            
            // 500 hatası yerine BadRequest döndür (exception handling middleware'e düşmesin)
    ...
    public async Task<ActionResult> Heartbeat([FromBody] HeartbeatRequest request)
    {
        ...
        _logger.LogInformation("💓 Heartbeat alındı: DeviceId={DeviceId}, IpAddress={IpAddress}", 
        ...
        try
        {
            ...
            _logger.LogDebug("💓 LastSeen güncellendi: DeviceId={DeviceId}", request.DeviceId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ LastSeen güncelleme hatası: DeviceId={DeviceId}", request.DeviceId);
        ...
        try
        {
            ...
            _logger.LogInformation("💓 Heartbeat işlendi: DeviceId={DeviceId}, IsOnline={IsOnline}", request.DeviceId, isOnline);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ IsOnline kontrol hatası: DeviceId={DeviceId}", request.DeviceId);
        }
